...
The full probe catalog (A–G) is built and reviewed. `--agentic` (category A) has been built and verified with a scripted fake driver client at zero API cost, but has never been run against a real model — no `ANTHROPIC_API_KEY` was available while this was built. Not yet done: a demo GIF (tooling gap, see above), a Show HN launch post, and broader real-world validation beyond this project's own fixture pair.
